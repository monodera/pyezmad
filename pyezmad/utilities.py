...
# linelist = read_emission_linelist(wavelength='air')

def read_emission_linelist(wavelength='air'):
    path_to_database = os.path.join(pyezmad.__path__[0], 'database/emission_lines.dat')
    # print(path_to_database)
    linelist_all = np.genfromtxt(path_to_database, dtype=None, names=['line', 'w_air', 'w_vac'])
    linelist_air = {}
    linelist_vac = {}
    ...
    if wavelength == 'air':
        ...
        raise("wavelength=%s is not supported." % wavelength)
